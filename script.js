...
(function () {
  ...
  /* ── شريط تقدّم القراءة + بارالاكس خفيف للصورة ── */
  ...
  function onScroll() {
    ...
    requestAnimationFrame(function () {
      ...
      if (heroImg && !reduce && window.innerWidth > 860) {
        var y = Math.min(window.scrollY, window.innerHeight) * 0.08;
  ...
  if (btnCopy) {
    btnCopy.addEventListener('click', function () {
      ...
      if (navigator.clipboard && window.isSecureContext) {
        ...
        try { document.execCommand('copy'); say('تم نسخ الرابط'); } catch (err) { say(url); }
...
(function () {
  ...
  var idx = 0, userToggled = false, fadeTimer = null, gapTimer = null;
  ...
  // المتصفحات بتمنع الصوت قبل أول تفاعل — بنفضل مستنيين ونشغّل مع أول لمسة
  ...
  function tryStart() {
    ...
    var p = audio.play();
    if (p && p.then) p.then(function () { fadeTo(TARGET_VOL); disarm(); }).catch(function () {});
    else { fadeTo(TARGET_VOL); disarm(); }
  }
  ...
  window.__weddingPlay = function () { userToggled = true; play(); };
...
(function () {
  ...
  if (to) {
    ...
    if (line) {
      line.textContent = 'دعوة خاصة إلى ';
  ...
  btn.addEventListener('click', function enter() {
    ...
    setTimeout(function () { opener.style.display = 'none'; }, 1400);
  });
})();
